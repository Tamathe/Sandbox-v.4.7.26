...
import { Menu, X, ChevronDown, Compass, Hammer, Sparkles, Send, Brain, Shield, Database, BookMarked, Building2, MessageSquare, Target, FileText, Code2, Bell, FlaskConical } from 'lucide-react'
...
export default function Header() {
  ...
  const [studentChallengeCount, setStudentChallengeCount] = useState(0)
  const [notificationCount, setNotificationCount] = useState(0)
  const pendingChallengeCount = currentUser.role === 'STUDENT' ? studentChallengeCount : 0
  ...
  useEffect(() => {
    ...
    const fetchChallenges = async () => {
      try {
        ...
        const data = await response.json()
        if (!cancelled) {
          setStudentChallengeCount(Array.isArray(data.pending) ? data.pending.length : 0)
        }
      } catch {}
    }
  ...
  const NAV_ITEMS: NavLink[] = [
    ...
    { href: '/academy', label: 'Academy', always: true },
    { href: '/hub', label: 'Hub', always: true },
    { href: '/campus', label: 'Campus', always: true },
    { href: '/studio', label: 'Studio', always: true },
  ]

  const quickLinks = [
    // Academy
    { href: '/courses', label: 'My Courses', icon: BookMarked, always: true },
    { href: '/library', label: 'My Library', icon: BookMarked, roles: ['STUDENT'] },
    { href: '/research-hub', label: 'Research Hub', icon: FlaskConical, roles: ['EDUCATOR', 'ADMIN'] },
    ...
    { href: '/avatar', label: 'My TA', icon: Brain, roles: ['EDUCATOR', 'ADMIN'] },
    // Studio
    { href: '/builder', label: 'Open Builder', icon: Sparkles, always: true },
    { href: '/playground', label: 'Playground', icon: Code2, always: true },
    { href: '/my-apps', label: 'My Apps', icon: Code2, always: true },
    { href: '/publish', label: 'Publish Tool', icon: Send, always: true },
  ...
  const visibleNotificationCount = pathname.startsWith('/notifications') ? 0 : notificationCount
        ...
        <div className="flex items-center justify-between h-16">
          <div className="flex items-center gap-3 flex-shrink-0">
            ...
            <Link href="/" className="flex flex-col">
              <div className="font-bold text-[#0033A0] text-xl leading-tight">The Core</div>
            ...
            >
              <span className="relative inline-flex">
                ...
                {visibleNotificationCount > 0 && (
                  <span className="absolute -right-2 -top-2 inline-flex min-w-[18px] items-center justify-center rounded-full bg-[#0033A0] px-1.5 py-0.5 text-[10px] font-bold text-white">
                    {visibleNotificationCount > 9 ? '9+' : visibleNotificationCount}
                  </span>
                )}
              </span>
              <span>Alerts</span>
            ...
            >
              <span className="flex items-center gap-3">
                <span className="relative inline-flex h-8 w-8 items-center justify-center rounded-xl bg-gray-100">
                  <Bell className="h-4 w-4" />
                  {visibleNotificationCount > 0 && (
                    <span className="absolute -right-1 -top-1 inline-flex min-w-[18px] items-center justify-center rounded-full bg-[#0033A0] px-1.5 py-0.5 text-[10px] font-bold text-white">
                      {visibleNotificationCount > 9 ? '9+' : visibleNotificationCount}
                    </span>
                  )}
                </span>
