...
import { Send, X, ChevronRight, Bot, FileText, Loader2, Mic, ExternalLink, Rocket, RotateCcw } from 'lucide-react'
...
export default function ConciergePanel() {
  ...
  const submit = useCallback(async (content: string) => {
    ...
    try {
      ...
      // Fix 13: notify NotebookWidget to refresh when Sandy saves a note
      if (fullText.includes('<!--SAVE_NOTE:')) {
        window.dispatchEvent(new CustomEvent('sandbox-note-saved'))
      }
    } catch (err) {
  ...
  // Keep the continuous-mode transcript callback up to date with current closures
  useEffect(() => {
    continuousTranscriptCallbackRef.current = (text: string) => {
      if (!text.trim()) return
      // Ensure Sandy panel is visible so the user sees the response
      setOpen(true)
      setMobileOpen(true)
      // If Sandy is mid-response, buffer the utterance and submit when free
      if (isLoading) {
        pendingVoiceRef.current = text
      } else {
        void submit(text)
      }
      // Restart recognition after a brief pause if still in continuous mode
      setTimeout(() => {
  ...
  }, [submit, startRecording, isLoading])

  // Flush any buffered voice utterance once Sandy finishes responding
  useEffect(() => {
    if (!isLoading && pendingVoiceRef.current) {
  ...
  // Pointer-event handlers for the mic button
  // - Quick tap  → toggle continuous listening on/off
  // - Hold (≥250ms) → push-to-talk; release stops and delivers transcript to input
  const handleMicPointerDown = useCallback((_e: React.PointerEvent) => {
    // Always start a hold timer so pointerUp can detect click vs hold regardless of mode.
    // Only start PTT recording when the hold fires AND we're not already in continuous mode.
  ...
  const handleMicPointerUp = useCallback(() => {
    ...
    if (isPttActiveRef.current) {
      // PTT release → stop and deliver transcript
      isPttActiveRef.current = false
      stopRecording()
    } else if (wasClick) {
      // Quick click → toggle continuous mode
      if (continuousModeRef.current) {
  ...
  const handleMicPointerLeave = useCallback(() => {
    // If user drags off the button mid-hold, cancel or stop PTT — do not toggle continuous
    if (isPttActiveRef.current) {
      isPttActiveRef.current = false
      stopRecording()
    }
  ...
  const panelContent = (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 px-4 py-3 bg-gradient-to-r from-[#0033A0] to-purple-700 flex-shrink-0">
        ...
        {messages.length > 0 && (
          <button
            onClick={() => { setMessages([]); setRawMessages(new Map()); setInput('') }}
      ...
      <div className="flex-shrink-0 border-t border-gray-200 p-3 bg-white">
        <form onSubmit={e => { e.preventDefault(); submit(input) }} className="flex items-end gap-2">
          <textarea
            ...
            placeholder={isRecording || continuousMode ? 'Listening...' : 'Ask Sandy anything...'}
            disabled={isLoading}
            rows={1}
          ...
          {isSupported && (
            <button
              ...
              className="w-9 h-9 rounded-xl flex items-center justify-center transition-colors flex-shrink-0 relative bg-gray-100 text-gray-500 hover:bg-gray-200 select-none"
            >
