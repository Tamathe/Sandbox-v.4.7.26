...
import { ArrowLeft, BookMarked, Plus, Search } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 mb-6">
          <Link href="/" className="w-8 h-8 flex items-center justify-center rounded-lg text-gray-400 hover:text-gray-700 hover:bg-white border border-gray-200 transition-colors">
            <ArrowLeft className="w-4 h-4" />
          </Link>
          <div className="flex-1">
            <h1 className="text-xl font-bold text-gray-900 flex items-center gap-2">
              <BookMarked className="w-5 h-5 text-[#0033A0]" />
              My Notebook
            </h1>
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-[#0033A0] text-white rounded-xl text-sm font-medium hover:bg-[#002580] transition-colors"
          >
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <BookMarked className="mx-auto mb-3 h-10 w-10 text-gray-200" />
            <p className="text-gray-500 font-medium">
              {search || filterCourseId ? 'No notes match your filters' : 'No notes yet'}
            </p>
            {!search && !filterCourseId && (
              <p className="text-sm text-gray-400 mt-1">
                Ask Sandy to save something, or create a note manually.
              </p>
            )}
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map(note => (
              <button
                ...
                className="w-full text-left bg-white rounded-2xl border border-gray-200 px-5 py-4 hover:border-[#0033A0]/40 hover:shadow-sm transition-all group"
              >
