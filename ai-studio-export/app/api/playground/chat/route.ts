...
const PLAYGROUND_SYSTEM_PROMPT = `You are a code generator for The Sandbox, an AI-powered educational tool platform at the University of Kentucky.
...
- NEVER generate HTML on the first message. Ask 2-3 short clarifying questions first (subject, content, specific features). Plain text only.
- NEVER generate HTML on the second message either. Acknowledge their answers, ask any remaining important question, and let them know you're ready to build. Plain text only.
- Only generate the HTML app on the third user message or later, once you have enough context to build something genuinely useful.
- If the user explicitly says "just build it", "go ahead", or "generate it now" at any point, you may generate the HTML immediately.
...
export async function POST(req: NextRequest) {
  try {
    ...
    const body = (await req.json()) as { messages?: unknown[]; currentCode?: string }
...
${anthropicMessages[lastUserIndex].content}`,
    ...
    const noCodeYet = !currentCode.trim()
    const belowThreshold = noCodeYet && userMessageCount < 3
    const systemPrompt = belowThreshold
      ? `${PLAYGROUND_SYSTEM_PROMPT}\n\n## CURRENT TURN\nThis is user message ${userMessageCount} of this conversation and no app has been built yet. Do NOT output any HTML. Ask clarifying questions only (plain text).`
      : PLAYGROUND_SYSTEM_PROMPT
    ...
    console.error('POST /api/playground/chat error:', error)
    return NextResponse.json({ error: 'Failed to process Playground request' }, { status: 500 })
  }
}
