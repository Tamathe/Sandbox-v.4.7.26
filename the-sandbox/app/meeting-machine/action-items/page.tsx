...
import { ArrowLeft, ListChecks, Loader2, Copy, Check, ChevronRight } from 'lucide-react'
...
export default function ActionItemsPage() {
  ...
  } = useMeetingMachineTool('action-items')

  const [mobileTab, setMobileTab] = useState<'actions' | 'sandy'>('sandy')
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center size-10 rounded-xl bg-[#0033A0]/10"><ListChecks className="size-5 text-[#0033A0]" /></div>
            <div>
              <h1 className="text-xl font-extrabold text-gray-900">Action Items</h1>
              <p className="text-sm text-gray-500">{pipelineData ? 'Sandy extracted items from your minutes — confirm them' : 'Extract every task, owner, and deadline from meeting notes'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="lg:hidden bg-white border-b border-gray-200">
        <div className="flex max-w-6xl mx-auto">
          <button type="button" onClick={() => setMobileTab('actions')} className={`flex-1 py-2.5 text-sm font-medium text-center transition-colors ${mobileTab === 'actions' ? 'text-[#0033A0] border-b-2 border-[#0033A0]' : 'text-gray-500'}`}>Actions</button>
      ...
      <div className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" style={{ minHeight: 'calc(100vh - 220px)' }}>
          <div className={`lg:col-span-7 bg-white border-2 border-gray-200 rounded-2xl p-6 overflow-y-auto ${mobileTab !== 'actions' ? 'hidden lg:block' : ''}`}>
            {output ? (
              <>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <CopyBtn text={output} />
                    <button type="button" onClick={() => saveToPipeline('follow-up-drafter')} className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-[#0033A0] text-white hover:bg-[#002580] transition-colors">
                      Draft Follow-ups <ChevronRight className="size-3.5" />
                    </button>
                  </div>
                </div>
              ...
              <div className="flex flex-col items-center justify-center h-64 gap-3"><Loader2 className="size-8 text-[#0033A0] animate-spin" /><p className="text-sm text-gray-400">Extracting action items...</p></div>
            ) : (
              <div className="flex flex-col items-center justify-center h-64 text-center"><ListChecks className="size-10 text-gray-200 mb-3" /><p className="text-sm text-gray-400">Paste your meeting notes and Sandy will extract every action item</p></div>
            )}
          </div>
          ...
          <div className={`lg:col-span-5 bg-white border-2 border-gray-200 rounded-2xl overflow-hidden flex flex-col ${mobileTab !== 'sandy' ? 'hidden lg:flex' : ''}`} style={{ minHeight: 400 }}>
            <SandyInterviewPanel messages={messages} chips={chips} isSandyTyping={isSandyTyping} stepCount={2} currentStep={output ? 1 : 0} onSendMessage={sendMessage} onChipSelect={selectChip} onStartOver={startOver} placeholder="Paste notes or tell Sandy about the meeting..." disabled={isSandyTyping} />
          </div>
        </div>
      </div>
