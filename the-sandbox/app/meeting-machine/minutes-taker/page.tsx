...
import { ArrowLeft, NotebookPen, Loader2, Copy, Check, ChevronRight } from 'lucide-react'
...
export default function MinutesTakerPage() {
  const {
  ...
  } = useMeetingMachineTool('minutes-taker')

  const [mobileTab, setMobileTab] = useState<'minutes' | 'sandy'>('sandy')
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center size-10 rounded-xl bg-[#0033A0]/10"><NotebookPen className="size-5 text-[#0033A0]" /></div>
            <div>
              <h1 className="text-xl font-extrabold text-gray-900">Minutes Taker</h1>
              <p className="text-sm text-gray-500">{pipelineData ? 'Sandy has your agenda — let\'s capture what happened' : 'Turn messy notes into structured minutes'}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="lg:hidden bg-white border-b border-gray-200">
        <div className="flex max-w-6xl mx-auto">
          <button type="button" onClick={() => setMobileTab('minutes')} className={`flex-1 py-2.5 text-sm font-medium text-center transition-colors ${mobileTab === 'minutes' ? 'text-[#0033A0] border-b-2 border-[#0033A0]' : 'text-gray-500'}`}>Minutes</button>
      ...
      <div className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" style={{ minHeight: 'calc(100vh - 220px)' }}>
          <div className={`lg:col-span-7 bg-white border-2 border-gray-200 rounded-2xl p-6 overflow-y-auto ${mobileTab !== 'minutes' ? 'hidden lg:block' : ''}`}>
            {output ? (
              <>
                <div className="flex items-center justify-between mb-4">
                  <h2 className="font-bold text-gray-900">Meeting Minutes</h2>
                  <div className="flex items-center gap-2">
                    <CopyBtn text={output} />
                    <button type="button" onClick={() => saveToPipeline('action-items')} className="inline-flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-[#0033A0] text-white hover:bg-[#002580] transition-colors">
                      Extract Actions <ChevronRight className="size-3.5" />
                    </button>
                  </div>
                </div>
                <div className="prose prose-sm max-w-none text-gray-800"><DynamicMarkdown components={mdComponents}>{output}</DynamicMarkdown></div>
              </>
            ) : isGenerating ? (
              <div className="flex flex-col items-center justify-center h-64 gap-3"><Loader2 className="size-8 text-[#0033A0] animate-spin" /><p className="text-sm text-gray-400">Structuring your minutes...</p></div>
            ) : (
              <div className="flex flex-col items-center justify-center h-64 text-center"><NotebookPen className="size-10 text-gray-200 mb-3" /><p className="text-sm text-gray-400">Tell Sandy what happened in your meeting</p></div>
            )}
          </div>
          ...
          <div className={`lg:col-span-5 bg-white border-2 border-gray-200 rounded-2xl overflow-hidden flex flex-col ${mobileTab !== 'sandy' ? 'hidden lg:flex' : ''}`} style={{ minHeight: 400 }}>
            <SandyInterviewPanel messages={messages} chips={chips} isSandyTyping={isSandyTyping} stepCount={2} currentStep={output ? 1 : 0} onSendMessage={sendMessage} onChipSelect={selectChip} onStartOver={startOver} placeholder="Paste your notes or describe the meeting..." disabled={isSandyTyping} />
          </div>
        </div>
      </div>
