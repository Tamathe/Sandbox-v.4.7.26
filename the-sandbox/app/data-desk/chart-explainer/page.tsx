...
import { ArrowLeft, BarChart3, Loader2, Copy, Check, Upload, X, ImageIcon } from 'lucide-react'
...
export default function ChartExplainerPage() {
  const {
  ...
  } = useDataDeskTool('chart-explainer')
  ...
  const handleFile = useCallback((f: File) => {
    if (f.type.startsWith('image/')) setInputFile(f)
  }, [setInputFile])
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <div className="bg-white border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center size-10 rounded-xl bg-[#0033A0]/10"><BarChart3 className="size-5 text-[#0033A0]" /></div>
            <div><h1 className="text-xl font-extrabold text-gray-900">Chart Explainer</h1><p className="text-sm text-gray-500">Upload any chart and Sandy analyzes it from your angle</p></div>
          </div>
        </div>
      </div>
            ...
            >
              ...
              {inputFile ? (
                <div className="flex flex-col items-center gap-3">
                  <ImageIcon className="size-10 text-[#0033A0]" />
                  <p className="text-sm font-medium text-gray-900">{inputFile.name}</p>
                  <div className="flex gap-2">
                    <button type="button" onClick={(e) => { e.stopPropagation(); submitData() }} className="px-4 py-2 rounded-xl bg-[#0033A0] text-white text-sm font-semibold hover:bg-[#002580] transition-colors">Analyze This Chart</button>
                ...
                <div className="flex flex-col items-center gap-3">
                  ...
                  <p className="text-sm font-medium text-gray-600">Drop a chart image here, or click to browse</p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" style={{ minHeight: 'calc(100vh - 220px)' }}>
            <div className={`lg:col-span-7 overflow-y-auto ${mobileTab !== 'analysis' ? 'hidden lg:block' : ''}`}>
              {output ? <SectionedOutput output={output} /> : isGenerating ? (
                <div className="bg-white border-2 border-gray-200 rounded-2xl flex items-center justify-center h-64"><Loader2 className="size-8 text-[#0033A0] animate-spin" /><p className="text-sm text-gray-400 ml-3">Analyzing your chart...</p></div>
              ) : null}
            </div>
            <div className={`lg:col-span-5 bg-white border-2 border-gray-200 rounded-2xl overflow-hidden flex flex-col ${mobileTab !== 'sandy' ? 'hidden lg:flex' : ''}`} style={{ minHeight: 400 }}>
              <SandyInterviewPanel messages={messages} chips={chips} isSandyTyping={isSandyTyping} stepCount={2} currentStep={output ? 1 : 0} onSendMessage={sendMessage} onChipSelect={selectChip} onStartOver={startOver} placeholder="Ask Sandy about the chart..." disabled={isSandyTyping} />
            </div>
          </div>
        )}
      </div>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pb-6">
        <p className="text-center text-xs text-gray-400">Data Desk provides AI-generated analysis — always verify findings against original data.</p>
      </div>
    </div>
  )
}
