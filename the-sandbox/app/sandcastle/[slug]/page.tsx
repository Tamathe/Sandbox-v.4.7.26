...
import { ArrowLeft, Send, Coins, Loader2, Mic, MicOff, MapPin } from 'lucide-react'
...
export default function SandcastleExperiencePage() {
  ...
  useEffect(() => {
    fetch(`/api/xp?email=${encodeURIComponent(currentUser.email)}`)
      .then(r => r.json())
      .then(d => { if (d.sandBalance !== undefined) setSandBalance(d.sandBalance) })
      .catch(() => {})
  }, [currentUser.email])
  ...
  if (!experience) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <Link href="/sandcastle" className="inline-flex items-center gap-2 bg-amber-500 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-amber-600 transition-colors">
  ...
  const canAfford = sandBalance === null || sandBalance >= experience.sandCost
  ...
  if (!canAfford) {
    return (
      <div className="max-w-2xl mx-auto px-4 py-20 text-center">
        ...
        <div className="inline-flex items-center gap-2 bg-amber-100 border border-amber-200 rounded-xl px-5 py-3 mb-6">
          <Coins className="w-5 h-5 text-amber-600" />
          <span className="font-semibold text-amber-800">Requires {experience.sandCost} Sand</span>
          <span className="text-amber-600 text-sm">(You have {sandBalance?.toLocaleString() ?? '...'})</span>
        </div>
        <div className="block" />
        <Link href="/sandcastle" className="inline-flex items-center gap-2 bg-amber-500 text-white px-5 py-2.5 rounded-xl font-semibold hover:bg-amber-600 transition-colors">
          <ArrowLeft className="w-4 h-4" />
      ...
      >
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            ...
            {experience.sandCost > 0 && (
              <span className="flex items-center gap-1 bg-white/20 border border-white/30 rounded-full px-2.5 py-1 text-xs font-semibold text-amber-100">
                <Coins className="w-3 h-3" />
                {experience.sandCost} Sand
              </span>
            )}
            {sandBalance !== null && (
              <span className="flex items-center gap-1 bg-white/10 rounded-full px-2.5 py-1 text-xs text-amber-200">
                <Coins className="w-3 h-3" />
                {sandBalance.toLocaleString()} bal.
              </span>
            )}
          </div>
        </div>
      ...
      {messages.length === 1 && experience.starterPrompts.length > 0 && (
        <div className="flex-shrink-0 bg-gray-50 border-t border-gray-100 px-4 py-2">
          <div className="max-w-3xl mx-auto flex flex-wrap gap-2">
            {experience.starterPrompts.map((prompt, i) => (
              <button
                key={i}
