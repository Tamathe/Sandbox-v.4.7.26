...
import { ArrowLeft, CheckSquare, Loader2, Copy, Check } from 'lucide-react'
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center size-10 rounded-xl bg-[#0033A0]/10">
              <CheckSquare className="size-5 text-[#0033A0]" />
            </div>
            <div>
              <h1 className="text-xl font-extrabold text-gray-900">Habit Tracker</h1>
              <p className="text-sm text-gray-500">Sandy helps you build daily habits with a quick check-in</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 max-w-6xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6" style={{ minHeight: 'calc(100vh - 220px)' }}>
          <div className={`lg:col-span-7 bg-white border-2 border-gray-200 rounded-2xl p-6 overflow-y-auto ${mobileTab !== 'check-in' ? 'hidden lg:block' : ''}`}>
            {output ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 gap-3">
                <Loader2 className="size-8 text-[#0033A0] animate-spin" />
                <p className="text-sm text-gray-400">Analyzing your habits...</p>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center h-64 text-center">
                <CheckSquare className="size-10 text-gray-200 mb-3" />
                <p className="text-sm text-gray-400">Tell Sandy which habits you completed today</p>
              </div>
            )}
          </div>
          ...
          <div className={`lg:col-span-5 bg-white border-2 border-gray-200 rounded-2xl overflow-hidden flex flex-col ${mobileTab !== 'sandy' ? 'hidden lg:flex' : ''}`} style={{ minHeight: 400 }}>
            <SandyInterviewPanel messages={messages} chips={chips} isSandyTyping={isSandyTyping} stepCount={2} currentStep={output ? 1 : messages.length > 1 ? 1 : 0} onSendMessage={sendMessage} onChipSelect={selectChip} onStartOver={startOver} placeholder="Tell Sandy about your habits..." disabled={isSandyTyping} />
          </div>
        </div>
      </div>
