...
import { ArrowLeft, Moon, Loader2, Copy, Check } from 'lucide-react'
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center justify-center size-10 rounded-xl bg-[#0033A0]/10">
              <Moon className="size-5 text-[#0033A0]" />
            </div>
            <div>
              <h1 className="text-xl font-extrabold text-gray-900">Sleep Log</h1>
              <p className="text-sm text-gray-500">Sandy helps you track sleep patterns with a quick morning check-in</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <div className={`lg:col-span-7 bg-white border-2 border-gray-200 rounded-2xl p-6 overflow-y-auto ${mobileTab !== 'check-in' ? 'hidden lg:block' : ''}`}>
            {output ? (
              ...
              <div className="flex flex-col items-center justify-center h-64 gap-3">
                <Loader2 className="size-8 text-[#0033A0] animate-spin" />
                <p className="text-sm text-gray-400">Analyzing your sleep patterns...</p>
              </div>
            ) : (
              <div className="flex flex-col items-center justify-center h-64 text-center">
                <Moon className="size-10 text-gray-200 mb-3" />
                <p className="text-sm text-gray-400">Answer Sandy&apos;s questions about last night&apos;s sleep</p>
              </div>
            )}
          </div>
