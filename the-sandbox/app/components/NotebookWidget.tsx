...
import { ArrowRight, BookMarked, FileText, Plus } from 'lucide-react'
...
export default function NotebookWidget({ userEmail }: NotebookWidgetProps) {
  ...
  useEffect(() => {
    ...
    window.addEventListener('uky-note-saved', handler)
    return () => window.removeEventListener('uky-note-saved', handler)
  }, [userEmail])
    ...
    <>
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-1.5">
          <FileText className="size-3.5 text-[#0033A0]" />
      ...
      {notes.length === 0 ? (
        <div className="rounded-xl border border-dashed border-gray-200 bg-gray-50 p-5 text-center">
          <BookMarked className="mx-auto mb-2 size-6 text-gray-300" />
          <p className="text-xs font-medium text-gray-500">Your notes will appear here</p>
          <p className="text-xs text-gray-400 mt-0.5">Ask Sandy to save something, or create one now.</p>
          <button
            onClick={() => setEditorNote(null)}
