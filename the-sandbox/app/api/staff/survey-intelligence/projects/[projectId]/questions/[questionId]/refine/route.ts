...
import { getRefinePrompt, parseGenerationOutput } from '../../../../../../../../lib/staff/survey-intelligence-service'
...
export const POST = withErrorHandling(async (req: NextRequest, { params }: { params: Promise<{ projectId: string; questionId: string }> }) => {
  ...
  const parsed = await parseRequestBody<{ instruction: string; currentDraft: string }>(req)
  if ('error' in parsed) return parsed.error
  const body = parsed.data
  if (!body.instruction || !body.currentDraft) {
    return NextResponse.json({ error: 'instruction and currentDraft are required' }, { status: 400 })
  }
  ...
  // Re-search evidence for safety (instruction may imply new evidence needed)
  ...
  // Build prompt
  const systemPrompt = getRefinePrompt({
    ...
    additionalEvidence: vaultResults,
    additionalUKNow: ukNowResults,
  })
    ...
    new ReadableStream({
      async start(ctrl) {
        try {
          ...
          // After stream completes, persist the refined draft
