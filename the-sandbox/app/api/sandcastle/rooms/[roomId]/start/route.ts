...
import { startRoom } from '../../../../../lib/sandcastle/room-service'
...
export const POST = withErrorHandling(async (
  ...
  // Verify host ownership before delegating
  ...
  try {
    const result = await startRoom(roomId, auth.user.id)
    return NextResponse.json(result)
  } catch (err: unknown) {
    const e = err as { code?: string; status?: number; message?: string }
    return NextResponse.json(
      { error: e.message ?? 'Failed to start room', code: e.code },
      { status: e.status ?? 500 },
    )
  }
})
