...
function buildFallbackPrompt(personaName?: string, toolName?: string) {
  ...
  return `You are ${displayName}, the AI tutor for ${displayTool} at the University of Kentucky. Teach clearly, stay encouraging, and help the learner think step by step instead of jumping straight to the answer.`
}
...
export const POST = withErrorHandling(async (request: NextRequest) => {
  ...
  const rateLimitError = await checkRateLimit(request, auth.user.id, 'CHAT', auth.user.role !== 'STUDENT')
